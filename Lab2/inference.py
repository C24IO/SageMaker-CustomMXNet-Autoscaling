...
def model_fn(model_dir):
    """
    Function for loading our model and returning a predictor object 
    """
    data_names = ['data', 'im_info']
    label_names = []

    logger.info("\nmodel_fn\n")
    provide_data = [[('data', (1, 3, 256, 256)), ('im_info', (1, 3))],
 [('data', (1, 3, 256, 25)), ('im_info', (1, 3))]]
    max_data_shape = [[('data', (1, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES])))]]
    provide_label = [None, None]
...
def transform_fn(predictor, img, input_content_type, output_content_type):
    """
    Prediction function, takes in protobuf content, and returns bounding box predictions
    """
    logger.info("\ntransform_fn\n")
